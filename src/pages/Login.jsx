  ...
  return (
    <div className=' mx-auto grid items-center w-[100%] h-[100vh] '>
        <div className='border grid w-[97%]  md:w-[25%] mx-auto h-[60vh] text-black'>
            <form onSubmit={(e) => login(e)} className='md:w-[80%] grid items-center mx-auto' >
                ...
                {footerLinks.loginInputs.map((input, index) => (
                    <div key={index} className='w-[100%] h-[5vh] border'>
                        <TextField onChange={(e) => dispatch(setUsername({input:input.name, value:e.target.value}))} fullWidth label={input.name}  type={input.type} />
                    </div>
                ))}
                <div className='w-[100%] text-center bg-black text-white  h-[5vh] border'>
                        <input type="submit" className='text-center focus:outline-0 p-2 w-[100%] h-[100%] bg-black p-2 ' value="login"/>
                 </div>
                </div>
                </div>
            </form>
            ...
            <div className='flex justify-between w-[80%] mx-auto'>
                ...
                <Link to="/Register">
                <p className='text-[#291E46] hover:font-bold hover:text-black'>Register</p>
                </Link>
            </div>
            </div>
        </div>
